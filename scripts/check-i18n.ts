#!/usr/bin/env ts-node
/**
 * i18n Validation Script for Obsidian Plugins
 * Checks:
...
function checkI18n(options: CheckOptions): boolean {
    ...
    for (const file of targetFiles) {
        ...
        let targetDict: Record<string, string>;
        try {
        ...
        for (const key of baseKeys) {
            ...
            const baseVal = baseDict[key] || key; // if baseVal empty, use key itself
